...
var hasBeddingCols=currentQuoteData.some(function(q){return q.items.some(function(it){return it.partName!==undefined||it.materialCode!==undefined||it.weight!==undefined||it.netUsage!==undefined||it.loss!==undefined;});});
if(hasBeddingCols){
var hc='<th>部件名称</th><th>物料名称</th><th>牌号</th><th>实际用量</th><th>单价</th><th>含税金额</th><th>小计</th><th>供应商</th>';
var allItems=[];
currentQuoteData.forEach(function(q){q.items.forEach(function(it){allItems.push({supplier:q.supplierName,partName:it.partName||'',materialName:it.materialName||'',materialCode:it.materialCode||'',actualUsage:((Number(it.netUsage)||0)*(1+(Number(it.loss)||0)/100)),unitPrice:it.unitPrice||0,subtotal:it.subtotal||0});});});
    ...
    currentQuoteData.forEach(function(q){
      ...
      if(sp&&sp.items){
        sp.items.forEach(function(it){
          var au=((Number(it.netUsage)||0)*(1+(Number(it.loss)||0)/100)).toFixed(2);
...
function switchMaterialTable(){
  ...
  // Reset all spec blocks - save template inner first
  initSpecTemplateInner();
  document.getElementById("bidSpecsContainer").innerHTML='<div class="spec-block" data-spec-idx="0">'+specTemplateInner+'<button class="btn btn-outline btn-sm" onclick="addBidItem(this)" style="margin-top:8px">+ 添加物料</button> <a href="#" onclick="downloadMaterialTemplate(event)" style="margin-left:12px;font-size:13px;color:#2563eb;text-decoration:underline;cursor:pointer">下载物料明细模板</a> <a href="#" onclick="uploadMaterialTemplate(event)" style="margin-left:8px;font-size:13px;color:#27ae60;text-decoration:underline;cursor:pointer">上传物料报价明细</a></div>';
  specCounter=1;
  renderTableHead();
  addBidItem(document.querySelector("#bidSpecsContainer .spec-block .btn"));
}
var specTemplateInner="";
function renderTableHead(){
document.querySelectorAll("#bidSpecsContainer .spec-items-head").forEach(function(head){
if(isBeddingMode){
...
function addBidItem(btnOrData){
var btnEl=(btnOrData instanceof HTMLElement||(btnOrData&&btnOrData.nodeType===1))?btnOrData:null;
var data=(btnEl||typeof btnOrData!=="object")?{}:btnOrData;
// Find nearest spec-block
var block=btnEl?btnEl.closest(".spec-block"):document.querySelector("#bidSpecsContainer .spec-block");
if(!block)return;
var tbody=block.querySelector(".spec-items-body");
if(!tbody)return;
...
var grandMaterialTotal=0,grandLabor=0,grandMfg=0,grandAdmin=0,grandProfit=0,grandTax=0;
document.querySelectorAll("#bidSpecsContainer .spec-block").forEach(function(block){
  var materialTotal=0;
  block.querySelectorAll(".spec-items-body tr").forEach(function(row){
    ...
    if(isBeddingMode){
      ...
      var elAct=row.querySelector(".item-actual");if(elAct)elAct.textContent=actualUsage.toFixed(2);
      var elTax=row.querySelector(".item-tax");if(elTax)elTax.textContent=taxAmount.toFixed(2);
      var elSub=row.querySelector(".item-subtotal");if(elSub)elSub.textContent=taxAmount.toFixed(2);
      materialTotal+=taxAmount;
    }else{
      var qty=Number(inputs[3]&&inputs[3].value)||0;
      var price=Number(inputs[5]&&inputs[5].value)||0;
      var subtotal=qty*price;
      var elSub2=row.querySelector(".item-subtotal");if(elSub2)elSub2.textContent=subtotal.toLocaleString();
      materialTotal+=subtotal;
    }
  });
  var mc=block.querySelector(".spec-material-cost");if(mc)mc.value=materialTotal.toFixed(2);
  var labor=Number((block.querySelector(".spec-labor")||{}).value)||0;
  var mfg=Number((block.querySelector(".spec-mfg")||{}).value)||0;
  var admin=Number((block.querySelector(".spec-admin")||{}).value)||0;
  var profit=Number((block.querySelector(".spec-profit")||{}).value)||0;
  var tax=Number((block.querySelector(".spec-tax")||{}).value)||0;
  var blockTotal=materialTotal+labor+mfg+admin+profit+tax;
  var cs=block.querySelector(".spec-cost-summary");if(cs)cs.textContent="\u00a5"+blockTotal.toLocaleString();
  grandMaterialTotal+=materialTotal;grandLabor+=labor;grandMfg+=mfg;grandAdmin+=admin;grandProfit+=profit;grandTax+=tax;
});
document.getElementById("bidTotalDisplay").textContent=(grandMaterialTotal+grandLabor+grandMfg+grandAdmin+grandProfit+grandTax).toLocaleString();
}
...
// ===== Multi-spec support =====
var specCounter=1;
function downloadMaterialTemplate(e){if(e)e.preventDefault();var csv="";if(isBeddingMode){csv="\u90E8\u4EF6\u540D\u79F0,\u7269\u6599\u540D\u79F0,\u7269\u6599\u724C\u53F7,\u514B\u91CD,\u89C4\u683C/\u5E45\u5BBD,\u51C0\u7528\u91CF,\u635F\u8017(%),\u7269\u6599\u5355\u4EF7\n\u88AB\u5957,\u7EAF\u68C9\u9762\u6599,BT-60S-C,120,2.3,2.5,3,45\n\u5E8A\u5355,\u7EAF\u68C9\u9762\u6599,CD-60S-C,120,2.5,2,3,35\n\u6795\u5957,\u7EAF\u68C9\u9762\u6599,ZT-60S-C,80,1.5,0.8,3,12\n\u88AB\u82AF,\u7FBD\u7ED2\u586B\u5145,BX-90R,200,1.5,3,2,120\n";}else{csv="\u7269\u6599\u540D\u79F0,\u7F16\u7801,\u89C4\u683C,\u6570\u91CF,\u5355\u4F4D,\u5355\u4EF7(\u00A5)\n\u793A\u4F8B\u7269\u6599,MAT-001,\u89C4\u683CA,10,\u4E2A,50\n";}var BOM="\uFEFF";var blob=new Blob([BOM+csv],{type:"text/csv;charset=utf-8"});var url=URL.createObjectURL(blob);var a=document.createElement("a");a.href=url;a.download="\u7269\u6599\u660E\u7EC6\u6A21\u677F.csv";a.click();URL.revokeObjectURL(url);}

function uploadMaterialTemplate(e){if(e)e.preventDefault();var inp=document.createElement("input");inp.type="file";inp.accept=".csv";inp.onchange=function(){var file=inp.files[0];if(!file)return;var reader=new FileReader();reader.onload=function(ev){var text=ev.target.result;var lines=text.split(/\r?\n/).filter(function(l){return l.trim();});if(lines.length<2)return toast("CSV格式不正确","error");var block=inp.closest(".spec-block")||document.querySelector("#bidSpecsContainer .spec-block");if(!block)return;var tbody=block.querySelector(".spec-items-body");if(!tbody)return;tbody.innerHTML="";for(var i=1;i<lines.length;i++){var cols=lines[i].split(",");if(cols.length<2)continue;var row=document.createElement("tr");if(isBeddingMode&&cols.length>=8){row.innerHTML='<td><input value="'+(cols[0]||"")+'"></td><td><input value="'+(cols[1]||"")+'"></td><td><input value="'+(cols[2]||"")+'"></td><td><input type="number" value="'+(cols[3]||"")+'" onchange="calcBidTotal()"></td><td><input type="number" step="any" value="'+(cols[4]||"")+'" onchange="calcBidTotal()"></td><td><input type="number" value="'+(cols[5]||"")+'" onchange="calcBidTotal()"></td><td><input type="number" value="'+(cols[6]||"")+'" style="width:60px" onchange="calcBidTotal()"></td><td class="item-actual">0</td><td><input type="number" value="'+(cols[7]||"")+'" onchange="calcBidTotal()"></td><td class="item-tax">0</td><td class="item-subtotal">0</td><td><button class="btn btn-outline btn-sm" onclick="var tr=this.closest(\'tr\');if(tr)tr.remove();calcBidTotal()" style="color:#e74c3c">\u00d7</button></td>';}else if(cols.length>=6){row.innerHTML='<td><input value="'+(cols[0]||"")+'"></td><td><input value="'+(cols[1]||"")+'"></td><td><input value="'+(cols[2]||"")+'"></td><td><input type="number" value="'+(cols[3]||"")+'" style="width:60px" onchange="calcBidTotal()"></td><td><input value="'+(cols[4]||"")+'" style="width:50px"></td><td><input type="number" value="'+(cols[5]||"")+'" onchange="calcBidTotal()"></td><td class="item-subtotal">0</td><td><button class="btn btn-outline btn-sm" onclick="var tr=this.closest(\'tr\');if(tr)tr.remove();calcBidTotal()" style="color:#e74c3c">\u00d7</button></td>';}else{continue;}tbody.appendChild(row);}calcBidTotal();toast("\u5df2\u5bfc\u5165"+ (lines.length-1) +"\u6761\u7269\u6599","success");};reader.readAsText(file,"utf-8");};inp.click();}

function initSpecTemplateInner(){
  var block=document.querySelector("#bidSpecsContainer .spec-block");
  if(block&&!specTemplateInner){
    var clone=block.cloneNode(true);
    var btns=clone.querySelectorAll("button");
    btns.forEach(function(b){b.remove();});
    var links=clone.querySelectorAll('a[onclick*="downloadMaterialTemplate"]');
    links.forEach(function(a){a.remove();});
    specTemplateInner=clone.innerHTML;
  }
}
function addNewSpec(){
  initSpecTemplateInner();
  ...
  div.setAttribute("data-spec-idx",specCounter);
  div.innerHTML=specTemplateInner;
  var btn=document.createElement("button");
  btn.className="btn btn-outline btn-sm";
  btn.textContent="+ \u6dfb\u52a0\u7269\u6599";
  btn.onclick=function(){addBidItem(this);};
  btn.style.marginTop="8px";
  div.appendChild(btn);
  var dl=document.createElement("a");dl.href="#";dl.onclick=function(e){downloadMaterialTemplate(e);};dl.style.cssText="margin-left:12px;font-size:13px;color:#2563eb;text-decoration:underline;cursor:pointer;white-space:nowrap";dl.textContent="下载物料明细模板";div.appendChild(dl);var ul=document.createElement("a");ul.href="#";ul.onclick=function(e){uploadMaterialTemplate(e);};ul.style.cssText="margin-left:8px;font-size:13px;color:#27ae60;text-decoration:underline;cursor:pointer";ul.textContent="上传物料报价明细";div.appendChild(ul);
  div.querySelectorAll("input").forEach(function(inp){
    inp.value=inp.type==="number"?"0":"";
  });
  div.querySelectorAll("tbody").forEach(function(tb){tb.innerHTML="";});
  var head=div.querySelector(".spec-items-head");
  if(head){
    if(isBeddingMode){
      head.innerHTML='<tr><th>\u90e8\u4ef6\u540d\u79f0</th><th>\u7269\u6599\u540d\u79f0</th><th>\u7269\u6599\u724c\u53f7</th><th>\u514b\u91cd</th><th>\u89c4\u683c/\u5e45\u5bbd</th><th>\u51c0\u7528\u91cf</th><th>\u635f\u8017(%)</th><th>\u5b9e\u9645\u7528\u91cf</th><th>\u7269\u6599\u5355\u4ef7</th><th>\u542b\u7a0e\u91d1\u989d</th><th>\u5c0f\u8ba1</th><th></th></tr>';
    }else{
...
function collectAllSpecs(){
  ...
  document.querySelectorAll("#bidSpecsContainer .spec-block").forEach(function(block){
    ...
    var profit=Number((block.querySelector(".spec-profit")||{}).value)||0;
    var tax=Number((block.querySelector(".spec-tax")||{}).value)||0;
    specs.push({name:name,items:items,laborCost:labor,manufacturingCost:mfg,adminCost:admin,profit:profit,tax:tax});
  });
  return specs;
}
...
var allSpecs=collectAllSpecs();
var items=[];
allSpecs.forEach(function(sp){sp.items.forEach(function(it){items.push(it);});});
...
var total=allSpecs.reduce(function(s,sp){
  var m=sp.items.reduce(function(ss,it){return ss+(Number(it.subtotal)||0);},0);
  return s+m+sp.laborCost+sp.manufacturingCost+sp.adminCost+sp.profit+sp.tax;
},0);
var body={
  ...
  total:total,
  laborCost:allSpecs.reduce(function(s,sp){return s+(sp.laborCost||0);},0),
  manufacturingCost:allSpecs.reduce(function(s,sp){return s+(sp.manufacturingCost||0);},0),
  adminCost:allSpecs.reduce(function(s,sp){return s+(sp.adminCost||0);},0),
  profit:allSpecs.reduce(function(s,sp){return s+(sp.profit||0);},0),
  tax:allSpecs.reduce(function(s,sp){return s+(sp.tax||0);},0),
...
initSpecTemplateInner();
var container=document.getElementById("bidSpecsContainer");
var defaultBlock='<div class="spec-block" data-spec-idx="0">'+specTemplateInner+'<button class="btn btn-outline btn-sm" onclick="addBidItem(this)" style="margin-top:8px">+ \u6dfb\u52a0\u7269\u6599</button></div>';
container.innerHTML=defaultBlock;
specCounter=1;
...
if(bidId){
  try{
    ...
    if(r.success&&r.data){
      ...
      if(b.category==="\u88ab\u5b50\u4ef6\u5957"){isBeddingMode=true;}
      ...
      if(specs.length>0){
        ...
        specs.forEach(function(sp,idx){
          var div=document.createElement("div");
          div.className="spec-block";
          div.setAttribute("data-spec-idx",idx);
          div.innerHTML=specTemplateInner;
          var btn=document.createElement("button");
          btn.className="btn btn-outline btn-sm";
          btn.textContent="+ \u6dfb\u52a0\u7269\u6599";
          ...
          btn.style.marginTop="8px";
          div.appendChild(btn);
          var ddl=document.createElement("a");ddl.href="#";ddl.onclick=function(e){downloadMaterialTemplate(e);};ddl.style.cssText="margin-left:12px;font-size:13px;color:#2563eb;text-decoration:underline;cursor:pointer;white-space:nowrap";ddl.textContent="下载物料明细模板";div.appendChild(ddl);var uul=document.createElement("a");uul.href="#";uul.onclick=function(e){uploadMaterialTemplate(e);};uul.style.cssText="margin-left:8px;font-size:13px;color:#27ae60;text-decoration:underline;cursor:pointer";uul.textContent="上传物料报价明细";div.appendChild(uul);
          container.appendChild(div);
          var sni=div.querySelector(".spec-name-input");
          if(sni)sni.value=sp.name||"";
          var sl=div.querySelector(".spec-labor");if(sl)sl.value=sp.laborCost||0;
          var sm=div.querySelector(".spec-mfg");if(sm)sm.value=sp.manufacturingCost||0;
          var sa=div.querySelector(".spec-admin");if(sa)sa.value=sp.adminCost||0;
          var sf=div.querySelector(".spec-profit");if(sf)sf.value=sp.profit||0;
          var st=div.querySelector(".spec-tax");if(st)st.value=sp.tax||0;
          var tbody=div.querySelector(".spec-items-body");
          (sp.items||[]).forEach(function(it){
            if(!tbody)return;
            var row=document.createElement("tr");
            if(isBeddingMode){
              row.innerHTML='<td><input value="'+(it.partName||"")+'"></td><td><input value="'+(it.materialName||it.name||"")+'"></td><td><input value="'+(it.materialCode||"")+'"></td><td><input type="number" value="'+(it.weight||"")+'" onchange="calcBidTotal()"></td><td><input type="number" step="any" value="'+(it.spec||it.width||"")+'" onchange="calcBidTotal()"></td><td><input type="number" value="'+(it.netUsage||"")+'" onchange="calcBidTotal()"></td><td><input type="number" value="'+(it.loss||"")+'" style="width:60px" onchange="calcBidTotal()"></td><td class="item-actual">0</td><td><input type="number" value="'+(it.unitPrice||"")+'" onchange="calcBidTotal()"></td><td class="item-tax">0</td><td class="item-subtotal">0</td><td><button class="btn btn-outline btn-sm" onclick="var tr=this.closest(\'tr\');if(tr)tr.remove();calcBidTotal()" style="color:#e74c3c">\u00d7</button></td>';
            }else{
              row.innerHTML='<td><input value="'+(it.name||"")+'"></td><td><input value="'+(it.materialCode||"")+'"></td><td><input value="'+(it.spec||"")+'"></td><td><input type="number" value="'+(it.qty||"")+'" style="width:60px" onchange="calcBidTotal()"></td><td><input value="'+(it.unit||"")+'" style="width:50px"></td><td><input type="number" value="'+(it.unitPrice||"")+'" onchange="calcBidTotal()"></td><td class="item-subtotal">0</td><td><button class="btn btn-outline btn-sm" onclick="var tr=this.closest(\'tr\');if(tr)tr.remove();calcBidTotal()" style="color:#e74c3c">\u00d7</button></td>';
            }
            tbody.appendChild(row);
          });
          specCounter=idx+1;
    ...
    if(prjRes.success&&prjRes.data){
      ...
      if(prj.attachments&&prj.attachments.length>0){
        ...
        prj.attachments.forEach(function(att){
          attHtml+="<div style=\x27display:flex;align-items:center;gap:8px;padding:4px 8px;background:#f1f5f9;border-radius:4px;margin:4px 0\x27><span style=\x27flex:1;font-size:12px\x27>"+att.originalFileName+"</span><a href=\x27"+att.url+"\x27 class=\x27btn btn-outline btn-sm\x27 style=\x27font-size:11px\x27 download>\U0001f4e5 \u4e0b\u8f7d</a></div>";
        });
