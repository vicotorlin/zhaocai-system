...
function closeModal(id){$(id).classList.remove("show");if(id==="createProjectModal"){cpFiles=[];cpChannelFiles=[];cpTechFiles=[];}if(id==="bidModal"){bidFiles=[];savedSpecs=[];var sa=document.getElementById("savedSpecsArea");if(sa)sa.style.display="none";}}
...
var cpTechFiles=[];
var bidFiles=[];savedSpecs=[];var sa=document.getElementById("savedSpecsArea");if(sa)sa.style.display="none";
...
}aasync function submitBid(){saveCurrentSpecSilent();
var items=collectBidItems();
...
total:savedSpecs.reduce(function(s,sp){var m=sp.items.reduce(function(ss,it){return ss+(Number(it.subtotal)||0);},0);return s+m+sp.laborCost+sp.manufacturingCost+sp.adminCost+sp.profit+sp.tax;},0),
laborCost:savedSpecs.reduce(function(s,sp){return s+(sp.laborCost||0);},0),
manufacturingCost:savedSpecs.reduce(function(s,sp){return s+(sp.manufacturingCost||0);},0),
adminCost:savedSpecs.reduce(function(s,sp){return s+(sp.adminCost||0);},0),
profit:savedSpecs.reduce(function(s,sp){return s+(sp.profit||0);},0),
tax:savedSpecs.reduce(function(s,sp){return s+(sp.tax||0);},0),
...
bidFiles=[];savedSpecs=[];var sa=document.getElementById("savedSpecsArea");if(sa)sa.style.display="none";
