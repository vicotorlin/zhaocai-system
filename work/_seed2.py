...
def api(method, path, body=None):
    ...
    if body:
        ...
        conn.request(method, path, body=body_bytes, headers={"Content-Type": "application/json; charset=utf-8"})
    else:
...
# Step 1: Users already created
# Step 2: Create projects
print("=== Creating projects ===")
projects_data = [
    {"name": "2026年夏季凉感被采购", "deadline": "2026-09-15", "category": "被子件套", "plan": "采购2000套凉感被"},
...
suppliers = [f"supplier{i}@test.com" for i in range(1, 6)]
...
for proj in created_pids:
    pid = proj["id"]
    for idx, (supp_acc, supp_name) in enumerate(zip(suppliers, supplier_names)):
        ...
        for j in range(4):
            ...
            items.append({
                "partName": parts_list[j], "materialName": materials_list[j],
                "materialCode": codes_list[j], "weight": weight, "spec": spec_list[j],
        ...
        r = api("POST", "/api/supplier/bid", {
            "account": supp_acc, "projectId": pid, "supplierAccount": supp_acc,
            "supplierName": supp_name,
            "items": [{k: v for k, v in it.items() if k != "specName"} for it in items],
            "total": total, "laborCost": labor, "manufacturingCost": mfg,
            "adminCost": admin, "profit": profit, "tax": tax,
            "specs": specs_data, "category": proj["category"]
        })
        if r.get("success"):
            print(f"  {r['data']['id']}: {supp_name} -> {pid} total={total:,.0f}")
        else:
            print(f"  FAIL: {pid} {supp_name}: {r.get('message')}")
