...
// ===== Spec management =====
var savedSpecs=[];
function saveCurrentSpecFn(){
try{
saveCurrentSpecSilentFn();
...
function saveCurrentSpecSilentFn(){
...
function getAllSpecsFn(){
saveCurrentSpecSilentFn();
return savedSpecs;
}
function renderSavedSpecsFn(){
...
return '<div style="margin:2px 0;padding:4px 8px;background:#f0f7ff;border-radius:4px;display:flex;justify-content:space-between"><span>'+sp.name+' ('+sp.items.length+'项物料, \u00a5'+matSum.toLocaleString()+')</span><span style="color:#e74c3c;cursor:pointer" onclick="savedSpecs.splice('+i+',1);renderSavedSpecsFn();">\u2715</span></div>';
}).join("");
}
